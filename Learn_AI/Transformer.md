...
- **Encoder-only ([BERT](./BERT.md) 계열)**: 문맥 이해에 강점. 분류, 질의응답 등에 사용.
- **Decoder-only ([GPT](./GPT.md) 계열)**: 텍스트 생성에 강점. [Generative AI](./GenerativeAI.md)의 기반.
- **Encoder-Decoder (T5, Original Transformer)**: 번역, 요약 등에 사용.
- **[Vision Transformer (ViT)](./ViT.md)**: 이미지를 패치(Patch) 단위로 쪼개어 트랜스포머에 입력하는 모델. [LeJEPA](./LeJEPA.md) 등의 백본으로 사용됩니다.
